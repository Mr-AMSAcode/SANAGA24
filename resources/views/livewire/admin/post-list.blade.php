<div class="min-h-screen bg-stone-50">
    <div class="max-w-7xl mx-auto px-6 py-10">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-black text-stone-900 tracking-tight">{{ __('All Posts') }}</h1>
                <p class="text-stone-500 text-sm mt-0.5">{{ __('Manage every article across all editors') }}</p>
            </div>
        ...
        @if (count($selected) > 0)
            <div class="mb-4 flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                <span class="text-sm font-semibold text-amber-800">
                    {{ trans_choice(':count post selected|:count posts selected', count($selected), ['count' => count($selected)]) }}
                </span>
                <button wire:click="bulkPublish"
                        wire:confirm="{{ __('Publish every selected draft?') }}"
                        class="px-4 py-1.5 text-xs font-bold text-white bg-green-600 hover:bg-green-700
                           rounded-lg transition-colors">
                    {{ __('Bulk Publish') }}
                </button>
                <button wire:click="bulkDelete"
                        wire:confirm="{{ __('Move selected posts to trash?') }}"
        ...
        <div class="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tr class="border-b border-stone-100 text-left text-xs text-stone-500 uppercase tracking-wider bg-stone-50">
                    ...
                    <th class="px-4 py-3 font-semibold">{{ __('Editor') }}</th>
                    <th class="px-4 py-3 font-semibold">
                        <button wire:click="sort('section')" class="flex items-center gap-1 hover:text-stone-800">
                            {{ __('Section') }} @if($sortBy==='section')<span>{{ $sortDir==='asc'?'↑':'↓' }}</span>@endif
                        </button>
                    </th>
                    <th class="px-4 py-3 font-semibold">
                        <button wire:click="sort('status')" class="flex items-center gap-1 hover:text-stone-800">
                            {{ __('Status') }} @if($sortBy==='status')<span>{{ $sortDir==='asc'?'↑':'↓' }}</span>@endif
                        </button>
                    </th>
                ...
                @forelse ($this->posts as $post)
                    <tr @class([
                        ...
                        <td class="px-4 py-3.5 max-w-xs">
                            ...
                            @if ($post->deleted_at)
                                <span class="text-[10px] text-red-500 font-semibold">{{ __('Trashed') }}</span>
                            @endif
                        </td>
                            ...
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                @else
                                    ...
                                    <button wire:click="forceDelete({{ $post->id }})"
                                            wire:confirm="{{ __("Permanently delete ':title'? This cannot be undone.", ['title' => addslashes($post->title)]) }}"
